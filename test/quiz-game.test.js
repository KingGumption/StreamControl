...
test('locks roster, separates platform IDs, hides answers, and enforces first answer', () => {
  ...
  for (const platform of ['twitch','youtube','tiktok']) chat('same', '!quiz join', platform);
  chat('same','!quiz join'); assert.equal(game.players.size,3);
  game.next(); chat('late','!quiz join'); assert.equal(game.players.size,3);
  ...
  chat('same',`!quiz ${correct+1}`); chat('same',`!quiz ${(correct+1)%4+1}`);
  assert.equal(game.players.get('twitch:same').answer,correct);
  game.next(); assert.equal(game.survivors().length,1);
  chat('same','!quiz join','youtube'); assert.equal(game.survivors().length,1);
  assert.equal(events.filter(e=>e.eventType==='player_eliminated').length,2);
  game.next(); chat('same',`!quiz ${game.deck[1].answer+1}`); game.next();
...
test('deadline rejects late answers even before timer fires and everyone can lose', () => {
  const { game, chat, late } = setup(); game.open(); chat('a','!quiz join'); game.next(); late();
  chat('a',`!quiz ${game.deck[0].answer+1}`);
...
test('timer eliminates missing answers and stop prevents pending resolution', () => {
  const { game, chat, expire } = setup(); game.open();chat('a','!quiz join');game.next();expire();assert.equal(game.phase,'completed');
  game.open();chat('a','!quiz join');game.next();game.stop();expire();assert.equal(game.phase,'idle');
});
...
test('quiz analytics count participation and outcomes with platform filters', () => {
  const { game, chat, events } = setup();game.open({questionCount:1});chat('a','!quiz join');game.next();chat('a',`!quiz ${game.deck[0].answer+1}`);game.next();
