...
The Polaroid feature shares this application's authenticated Streamer.bot connection. Configure its URL and optional password with `STREAMERBOT_WS_URL` and `STREAMERBOT_WS_PASSWORD` in `.env`. The reward name, camera source, rendering, overlay, Discord, avatar helper, and Twitch-link settings remain in `data/polaroid-config.json`. Environment variables `POLAROID_OBS_PASSWORD`, `POLAROID_CAMERA_SOURCE`, `POLAROID_REWARD_TITLE`, `POLAROID_REWARD_ID`, and `POLAROID_DISCORD_WEBHOOK` can override the corresponding settings. Cloud deployments set `POLAROID_REWARD_TITLE=Polaroid` explicitly and can reliably enable the Streamer.bot helpers independently of the ignored data file with `POLAROID_TWITCH_CHAT_ENABLED`, `POLAROID_TWITCH_CHAT_ACTION_NAME`, `POLAROID_AVATAR_RESOLVER_ENABLED`, and `POLAROID_AVATAR_RESOLVER_ACTION_NAME`.

Quiz Twitch avatar lookups and Polaroid share a six-hour, in-memory URL cache. Polaroid also caches downloaded avatar images for six hours (up to 256 images / 32 MB). Concurrent requests share work; failed lookups/downloads are retried on the next request. Both caches clear on restart. Identified live chat, commands, game participation, redemptions and supported follow/subscription/raid/cheer/gift/like/share events warm avatars in the background. Twitch can resolve missing URLs; YouTube and TikTok require an avatar URL in the event. Warming uses two concurrent jobs, a bounded backlog, and a one-minute per-user/URL retry interval; unavailable avatars never block interactions. Polaroid retains its Streamer.bot avatar resolver on a cache miss.
...
Choose 1?15 questions (default 10) and 5?120 seconds per answer (default 20), then open the lobby. Viewers from connected Twitch, YouTube and TikTok chats enter with `!join`. **Lock entries & start** fixes the roster for the entire game. Players answer with a single number: `1`, `2`, `3` or `4`. Only their first answer counts. Wrong answers and missed deadlines eliminate them, and eliminated players cannot rejoin. The host starts the first question; answer deadlines and subsequent questions run automatically on the server. Each next question waits for the answer reveal and every elimination batch, plus a short buffer. The OBS overlay stays blank until a lobby is opened and becomes blank again when stopped. Players keep answering after the configured question count. Choose any mix of 13 categories: franchise logos, game characters, movie posters, actor pictures, game descriptions, game locations, horror trivia, weapons and items, game villains, movie plots, actors and roles, TV and animation, and gaming trivia. The 1,040-question bank has 80 questions in each of the 13 categories, including 247 picture rounds and 793 text rounds. All original 500 questions are retained. Live games remember shown question IDs in the existing persistent database, prioritise unseen questions from the selected categories, then reuse the least recently shown questions when needed. Unshown lobby/deck questions are not consumed. Preview tests have independent history and never consume live questions. History begins with games played after this feature was installed. Both live games and preview tests use this same bank; the per-game question count remains 1?15 before sudden death. Sudden death draws unused questions from the selected categories, then reshuffles that pool once exhausted; it no longer switches to maths. The last survivor (including solo play) keeps answering until their first wrong or missed answer, then receives the winner screen. If multiple remaining players all go out in one round, the result is defeat. The compact overlay first shows the correct answer and vote counts, then replaces that panel with animated eliminations and the winner or defeat screen. Player cards preserve display-name casing and include supplied profile pictures (with a Twitch avatar lookup fallback). The winner screen shows correct answers in this run and all-time quiz wins, counted from persisted analytics using platform and stable user ID, including earlier wins. Initials appear if no profile picture is available.

The question bank is `src/quiz-questions.json`. Each question has a unique `id`, `category`, `text`, four `options`, a zero-based `answer`, and `difficulty` from 1 to 15. Picture questions also have `image.file`, optional fractional `[x,y,width,height]` `image.crop`, and the cropped `image.aspect`. Keep image files in `public/quiz-media` with neutral filenames and record their sources in `src/quiz-media-sources.json`. The overlay crops logos and posters to hide titles, and uses generic alt text. Source metadata and correct answers stay out of the question-state payload. The media source list provides attribution links for publisher artwork and Wikimedia photographs. Restart the service to load bank changes. Unseen questions are chosen across the bank's difficulty range and ordered from easier to harder; when the remaining unseen pool runs out mid-game, those fresh questions precede the reused portion. Option order is shuffled each game. The bank stays server-side and correct answers are only exposed after the deadline or host reveal. The catalogue in `public/admin-games.html` can be extended with additional game cards.
