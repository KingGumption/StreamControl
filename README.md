...
Choose 1?15 questions (default 10) and 5?120 seconds per answer (default 20), then open the lobby. Viewers from connected Twitch, YouTube and TikTok chats enter with `!join`. **Lock entries & start** fixes the roster for the entire game. Players answer with a single number: `1`, `2`, `3` or `4`. Only their first answer counts. Wrong answers and missed deadlines eliminate them, and eliminated players cannot rejoin. The host advances after each answer reveal; answer deadlines run automatically on the server. A round leaving exactly one survivor ends immediately in victory; that winner never has to answer another question. If several players survive the final question, they share victory. If everyone is eliminated in the same round, the result is defeat. Reveals show correct/wrong counts per option and missed answers, followed by animated batches showing every eliminated player and a dedicated winner or defeat screen.
